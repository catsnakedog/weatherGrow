...
public class NewsSpawn : MonoBehaviour
{
    ...
    {   
        startcor = NewsWrite() ;
        stopcor = NewsRemove() ;
        contentSize = newsContent.Length ;
    ...
    void Update()
    {
        float moveex =MoveSpeed*Time.deltaTime ;
     
        if(transform.position.x > removepos.transform.position.x)
        {
           this.transform.Translate(moveex,0f,0f) ; 
    ...
    IEnumerator NewsWrite()
    {
        for(int i=0; i < contentSize ; i++)
        {
          this.transform.GetComponent<Text>().text += newsContent[i] ;
    ...
    {
        while(this.transform.GetComponent<Text>().text.Length!=0)
